...
(defn stated-numerical-y-axis [prepped-data {::keys [bottom top]}]
  (let [[t [_bottom _top]] (get-in prepped-data [:extents :y 0])
        margin 0.025]
    (if (= :numerical t)
      (plotb/update-scale prepped-data :y :domain [bottom (+ (* margin top) top)])
      prepped-data)))
...
(defn stated-y-index [{::series/keys [series legend-spec]
                       ::keys [x-axis
                               y-axis
                               size
                               legend-label ;; new
...
(defn free-y-index [{::series/keys [series legend-spec]
                     ::keys [x-axis
                             y-axis
                             size
                             legend-label ;; new
